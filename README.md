...
Profiles are private by default. Users can enable public JSON/SVG output from
`/settings/profile`; leaderboard participation also makes the public profile visible. Public
routes expose aggregate usage only, never emails, upload tokens, prompts, completions, local file
paths, or raw conversation logs.

The README card is generated by:

```txt
GET /api/public/:slug.svg
```

The profile page also provides a ready-to-copy Markdown snippet:

```md
[![TokenBoard](https://<your-tokenboard-domain>/api/public/<slug>.svg)](https://<your-tokenboard-domain>)
```

The card editor stores normalized JSON in `profiles.public_card_config`. It supports Chinese and
English labels, light and dark themes, balanced/compact/wide layouts, configurable glow position
and intensity, custom title/subtitle text, public URL visibility, metric ordering, hidden metric
slots, live authenticated preview, and a one-click reset to defaults. Private profiles render a
sample preview in settings but the public JSON/SVG routes still respect the private visibility
boundary.
...
Node.js 22.12 or newer is required. The web workspace uses Wrangler 4.95 and Vite 8,
and that combination does not support Node 22.0 through 22.11.
